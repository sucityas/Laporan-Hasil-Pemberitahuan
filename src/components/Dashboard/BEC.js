...
class BEC extends Component {
    ...
    componentDidMount() {
        axios.get(process.env.REACT_APP_SCE_WS + `/tableau/get-token`, {headers: {
            ...
            .then(res => {
                ...
                const urlTableau  = 'https://analytics2.customs.go.id/views/DashboardImporBPSdanBC/DASHBOARDIMPORBPS_BCBEC';
                this.setState({
    ...
    render() {
        return (
            <div>
                <SubHeader subHeaderTitle="Dashboard Nasional" breadcrumbs="Dashboard Broad Economic Categories (BEC)"/>
