...
class DetailBarang extends Component {
        ...
        if (prevProps.viewData !== this.props.viewData || prevProps.editData !== this.props.editData) {
            ...
            if (!(this.props.viewData === null)) {
                ...
                this.setState({
                    ...
                    kodeIpl: this.props.viewData.kodeIpl,
                    iplAll: this.props.viewData.namaIpl,
    ...
    componentDidMount() {
        ...
        this.fetchReferensiInstrumen()
        // this.fetchReferensiInstrumen2()
    }
    ...
    // fetchReferensiInstrumen = () => {
    //     var valutaBarang = [{ "kodeIpl": "Rupiah"}, { "kodeIpl": "US Dollar"}, { "kodeIpl": "Australiain Dollar"}, { "kodeIpl": "Canadian Dollar"}, { "kodeIpl": "Danish Krone"}, { "kodeIpl": "Hong Kong Dollar" }, { "kodeIpl": "Malaysian Ringgit" }, { "kodeIpl": "New Zealand Dollar"}, { "kodeIpl": "Norwegian Krone" }, { "kodeIpl": "Pound Sterling" }];
    //     const result = valutaBarang.map(data => ({
    //       text: `${data.kodeIpl}`,
    //       value: data.kodeIpl,
    //     }));
    //     this.setState({ dataReferensiInstrumen: result, fetchingReferensiInstrumen: false });
    //   };
    ...
    fetchReferensiInstrumen = () => {
        ...
        const isLocalhost =
            window.location.host == "ceisa40.customs.go.id-prod";
        const res = axios.get(
            `https://apisdev-gw.beacukai.go.id/v3/barang-penumpang/referensi/jenis-ipl/list`,
            {
                headers: {
                    [!isLocalhost
                        ? "beacukai-api-key"
    ...
    onChangeTglAwal = (date, dateString) => {
        const tanggal = moment(dateString, 'DD-MM-YYYY')
        this.setState({
            tglAwal: tanggal,
        });
    };
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                {this.state.dataDokumen !== null ? (this.state.dataDokumen.header.kodeDokumen === "32" ? (
                    <Card
                        size="small"
                        title="Detail Barang"
                        extra={
                    ...
                    >
                        <Form>
                            ...
                            <Row gutter={8}>
                                ...
                                <Col span={20}>
                                    <Select
                                        placeholder="Pilih Uraian Barang"
                                        allowClear
                                        value={this.state.uraian}
                                        onChange={e => { this.setState({ uraian: e }); }}
                                        filterOption={(input, option) =>
                                            option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                          }
                                    >
                                        <Option value="Rupiah">Uang Kertas Rupiah</Option>
                                        <Option value="Asing">Uang Kertas Asing</Option>
                                        <Option value="Other">Instrumen Pembayaran Lainnya</Option>

                                    </Select>
                                </Col>
                            </Row>
                            {this.state.uraian==="Other"? 
                            <Row gutter={8}>
                                <Col span={4}>
                                    <p>Jenis Instrumen <span style={{ color: 'red' }}>*</span></p>
                                </Col>
                                <Col span={20}>
                                    <Select
                                        placeholder="Pilih Jenis Instrumen"
                                        showArrow={true}
                                        notFoundContent={this.state.fetchingReferensiInstrumen ? <Spin size="small" /> : null}
                                        showSearch
                                        style={{ width: "100%" }}
                                        optionFilterProp="children"
                                        readOnly={this.state.readOnly}
                                        disabled={this.state.readOnly}
                                        value={this.state.kodeIpl}
                                        onChange={e => { this.setState({ kodeIpl: e }); }}
                                        // value={this.state.searchKodeJenisPungutan}
                                        // onSearch={(value) => this.fetchKodeJenisPungutan(value)}
                                        filterOption={(input, option) =>
                                            option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                    ...
                                    >
                                        {/* {this.state.iplAll ?  (this.state.dataReferensiInstrumen2 !== undefined ? this.state.dataReferensiInstrumen2.map(d => (
                                            <Option key={d.value}>{d.text}</Option>
                                        )) : "") : */}
                                            {this.state.dataReferensiInstrumen !== undefined ? this.state.dataReferensiInstrumen.map(d => (
                                                <Option key={d.value}>{d.text
                                                }</Option>
                                        )) : ""}
                            ...
                            <Row gutter={8}>
                            <Col span={4}>
                                    <p>Nomor Dokumen BI<span style={{ color: 'red' }}>*</span></p>
                                </Col>
                                <Col span={8}>
                                    <Input
                                ...
                                <Col span={4} style={{paddingTop:"2px"}}> 
                                    <p>Tanggal Dokumen BI </p>
                                </Col>
                                <Col span={8}>
                                        <DatePicker
