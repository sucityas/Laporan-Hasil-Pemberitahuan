...
import PDFViewerLhp from "./pdfViewerLhp";
const { Option } = Select;
...
class index extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            page: 0,
            kodeProses: "'1000004','1000005','1000006','1000007'",
            listProses: null,
            visibleBap: false,
            visibleLhp: false
        };
        ...
        this.openBAP = this.openBAP.bind(this);
        this.openLHP = this.openLHP.bind(this);
    ...
    getSemuaData(e) {
        ...
        // };
        // console.log(body);
        axios.get(`https://apisdev-gw.beacukai.go.id/v3/barang-penumpang/table/header/multi-params?${param}`, {
            headers: {
                'beacukai-API-Key': `2f1313cf-e4e6-4172-926b-6ee720182f7a`,
            }
        })
                ...
                this.setState({
                    ...
                    if (!(this.state.dataDokumen === null || this.state.dataDokumen === undefined)) {
                        if (!(res.data.data.tdLhp === null || res.data.data.tdLhp === undefined)) {
                            this.setState({
                                idLhp: res.data.data.tdLhp.idLhp,
                            }, () => {
                                this.setState({
                                    visibleBap: true
                                })
                                // console.log(this.state.idLhp)
                            });
                        }
                    }
                });
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (key, record, e) => {
                    ...
                    return (
                        <div>
                            <button type="button" className="btn btn-bold btn-sm btn-label-primary" onClick={(evt) => this.openBAP(record, evt)}>
                                <span className="kt-hidden-mobile">BAP</span>
                            </button>
                            &nbsp;
                            <button type="button" className="btn btn-bold btn-sm btn-label-success" onClick={(evt) => this.openLHP(record, evt)}>
                                <span className="kt-hidden-mobile">LHP</span>
                            </button>
                            &nbsp;
                            <Link
                                to={"/barang-penumpang/lhp-hasil-periksa"}
                                type="primary"
                                onClick={(evt) => this.getidheader(record, evt)}
                            >
                                &nbsp;
                                <button type="button" className="btn btn-bold btn-sm btn-label-warning">
                                    <span className="kt-hidden-mobile">DETAIL</span>
                                </button>
        ...
        return (
            <Fragment>
                <div className="kt-subheader kt-grid__item" id="kt_subheader">
                    <div className="kt-subheader__main">
                        <Breadcrumb>
                            ...
                            <Breadcrumb.Item>
                                <Link to="/barang-penumpang/lhp-hasil">Pencarian Hasil Periksa </Link>
                            </Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
                </div>
                <Row gutter={8} className="mb-1">
                    <Card
                        title="Pencarian Hasil Pemeriksaan Barang"
                    >
                        <Form>
                ...
                <Modal
                    visible={this.state.visibleLhp}
                    title="Laporan Hasil Pemeriksaan"
                    width="60%"
                    onCancel={this.onCloseLhp}
                    onOk={this.onCloseLhp}
                // footer={[
                //     <Button key="back" onClick={() => handleModal(false, null)}>
                //         Tutup
                //     </Button>,
                ...
                >
                    <PDFViewerLhp
                        idLhp={this.state.idLhp} reload={this.state.visibleLhp}
                    />
                </Modal>
